...
install.packages(c("readxl", "xlsx","rJava", "WriteXLS", "OIsurv", "interval"))
library(rJava) 
library(WriteXLS) # Writing Excel files
library(readxl) # Excel file reading
library(xlsx) # Writing Excel files
setwd("C:/Users/amykr/Box Sync/ASTMH 2017 abstracts/elysse- apparent inapparent/data") # Always set where you're grabbing stuff from
survival <-  read.csv("elysse_survival.csv")
attach(survival)
survival$schikvigg 

#===> survival object <===#
# create a subset for just the first group by using [type==1]
my.surv.object <- Surv(firstvisit, right, schikvigg, type="interval", origin =1)
my.surv.object
...
library(devtools)
install_github("cran/interval")

icfit<-icfit(firstvisit, right, initp = NA, minerror = 1e-06, maxcount = 1000)
plot(icfit)

data(bcos)
icout<-icfit(Surv(firstvisit, right, schikvigg, type="interval", origin =1)~groups, data=my.surv.object)
plot(icout)
table(survival$groups, survival$schikvigg)

getsurv<-getsurv(right, icfit, nonUMLE.method = "interpolation")
