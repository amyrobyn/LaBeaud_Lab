...
ds <- read_stata("Nyangatom_sampleAMY_10-5-2018.DTA")
library(tidyverse)
library(survey)

growth<-ggplot(growth_long,aes(mean_length,mean_weight))
growth+geom_smooth(aes(mean_length,mean_weight),level=.95,size=2,formula=y~x,method="glm")+labs(title ="GLM", x = "Height (cm)", y = "Weight (Kg)") +  geom_point(aes(mean_length,mean_weight,color=visit))
#growth+geom_smooth(aes(mean_length,mean_weight),level=.95,size=2,formula=y~x,method="loess")+labs(title ="Loess", x = "Height (cm)", y = "Weight (Kg)") +  geom_point(aes(mean_length,mean_weight,color=visit))
growth+geom_smooth(aes(mean_length,mean_weight),level=.95,size=2,formula=y~x,method="lm")+labs(title ="LM", x = "Height (cm)", y = "Weight (Kg)") +  geom_point(aes(mean_length,mean_weight,color=visit))
...
fig3<-ggplot()+ 
  ...
  geom_smooth(data =ds,aes(height_cm,child_weight_kg, weight=sweight_child,color="Nyangatom\nLoess Regression\n& 95% CI\n", linetype="Nyangatom\nLoess Regression\n& 95% CI\n"),level=.95,size=1,formula=y~x,method="loess",alpha=.9) +
  ...
  scale_color_manual(name = "", values = c("Nyangatom\nLoess Regression\n& 95% CI\n"="black","WHO -2SD" = "black"), labels = c("Nyangatom\nLoess Regression\n& 95% CI\n","WHO -2SD"))+
  scale_fill_identity(name = '', guide = FALSE,labels = c('grey'='WHO median to -3SD')) +
  scale_linetype_manual(name = "",values = c("Nyangatom\nLoess Regression\n& 95% CI\n"=1, "WHO -2SD" = 3))+
...
# print to tiff -----------------------------------------------------------
tiff(file = "C:/Users/amykr/Box Sync/Amy's Externally Shareable Files/fig 3/fig3_arial12_MI_95CI_legend_dataoct52018_2.tiff", width = 6200, height = 3200, units = "px", res = 600)
  fig3
dev.off()
