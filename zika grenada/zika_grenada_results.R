...
names(preg_cohort) <- sub('ID.Code','mom_id_orig_study',names(preg_cohort))
ds <- join(ds, preg_cohort, by='mom_id_orig_study', type='left', match='all')

table(ds$interpreted_colors.1, exclude=NULL)
table(ds$interpreted_colors, exclude=NULL)
...
table(ds$interpreted_colors, ds$pregnant, exclude = NULL)
cases<-subset(ds, pregnant=="1" | interpreted_colors =="red")
